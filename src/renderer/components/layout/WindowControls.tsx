...
function Win11ControlButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-label={label}
      className={`w-[46px] h-8 flex items-center justify-center text-black/70 dark:text-white/70 transition-colors duration-75 ${hoverClass}`}
    >
      {children}
    </button>
  );
...
function WindowControlsInner({ embedded = false }: { embedded?: boolean }): React.JSX.Element {
  ...
  if (embedded) {
    return (
      <div className="flex items-start [app-region:no-drag] -mr-2 -mt-px">
        <div className="flex items-center">
          <Win11ControlButton onClick={handleMinimize} label="Minimize">
            <SvgIcon svg={minusSvg} size={10} />
          </Win11ControlButton>
          <Win11ControlButton onClick={handleToggleMaximize} label={isMaximized ? "Restore" : "Maximize"}>
            {isMaximized ? (
              <SvgIcon svg={CARDS_SVG} size={10} />
            ) : (
              <SvgIcon svg={SQUARE_SVG} size={9} />
            )}
          </Win11ControlButton>
          <Win11ControlButton onClick={handleClose} label="Close" variant="close">
